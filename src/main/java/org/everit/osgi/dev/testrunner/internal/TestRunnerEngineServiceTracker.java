...
public class TestRunnerEngineServiceTracker extends ServiceTracker<TestRunnerEngine, TestRunnerEngine> implements
    ...
    private Map<String, List<TestRunnerEngine>> testRunnersByEngineType = new HashMap<String, List<TestRunnerEngine>>();
    ...
    public TestRunnerEngine addingService(ServiceReference<TestRunnerEngine> reference) {
        ...
        TestRunnerEngine runnerEngine = bundleContext.getService(reference);

        WriteLock writeLock = mapRWLock.writeLock();
        writeLock.lock();
        List<TestRunnerEngine> testRunners = testRunnersByEngineType.get(testEngine);
        if (testRunners == null) {
            testRunners = new ArrayList<TestRunnerEngine>();
    ...
    @Override
    public void modifiedService(ServiceReference<TestRunnerEngine> reference, TestRunnerEngine service) {
        // Do nothing
    }

    @Override
    public void removedService(ServiceReference<TestRunnerEngine> reference, TestRunnerEngine service) {
        Object engineType = reference.getProperty(Constants.SERVICE_PROPERTY_TESTRUNNER_ENGINE_TYPE);
        String testEngine = (String) engineType;
    ...
    @Override
    public TestRunnerEngine getEngineByType(String testEngineType) {
